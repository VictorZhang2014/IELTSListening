...
public class ZQWaterFallViewLayout: UICollectionViewFlowLayout {
    ...
    //当前布局一共有两列
    public let collectionViewCellColumns: Int = 2
    ...
    public func layoutItem(at indexPath: IndexPath) {
        ...
        //如果当前的cell的只是collectionView的宽度的一半，或者除以columnHeightInList的宽度
        //找出高度最小的列
        ...
        //重新计算出cell的frame
        frame = CGRect(x: (edgeInsets?.left)! + CGFloat(columnIndex) * ((edgeInsets?.left)! + cellSize!.width), y: CGFloat(top) + (edgeInsets?.top)!, width: cellSize!.width, height: cellSize!.height)
